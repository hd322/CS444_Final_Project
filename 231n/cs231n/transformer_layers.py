...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        # position = [0, 1, 2, ..., last]
        term_j = torch.arange(0, embed_dim, 2) 
        # div_term = 10000 ** (term_j/embed_dim)
        div_term = torch.pow(10000, term_j.float() / embed_dim)
        # div = [0,0]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # N, H, S, D
        mh_q = q.view(N, S, self.n_head, self.head_dim).transpose(1, 2)
        # N, H, T, D
        mh_k = k.view(N, T, self.n_head, self.head_dim).transpose(1, 2)
        mh_v = v.view(N, T, self.n_head, self.head_dim).transpose(1, 2)

        # output = (N, H, S, T) <- (N, H, S, D) @ (N, H, D, T)
        dist = torch.matmul(mh_q, mh_k.transpose(-1, -2))
        dist = dist / math.sqrt(self.head_dim)

        if attn_mask is not None:
            dist = dist.masked_fill(attn_mask == 0, float('-inf'))
        
        attn_weight = torch.softmax(dist, dim=-1)
        attn_weight = self.attn_drop(attn_weight)

        # (N, H, S, D) <- (N, H, S, T) * (N, H, T, D)
        mh_out = torch.matmul(attn_weight, mh_v)

        # (N, S, H, D)
        output = mh_out.transpose(1, 2).contiguous().view(N, S, self.emd_dim)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(N, C, num_patch_h, self.patch_size, num_patch_w, self.patch_size)
        x = x.permute(0, 2, 4, 1, 3, 5)
        x = x.reshape(N, num_patch_h * num_patch_w, C * self.patch_size * self.patch_size)
